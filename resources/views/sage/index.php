<?php 
session_start();
require '../sage/database/db_connect.php';
// for header and banner logo
$select = "SELECT * FROM logos WHERE id=1";
$select_res = mysqli_query($db_connect,$select);
$after_assoc= mysqli_fetch_assoc($select_res);

// for user banner photo
$users = "SELECT * FROM users WHERE role=1";
$users_res = mysqli_query($db_connect,$users);
$users_after_assoc= mysqli_fetch_assoc($users_res);

//for abouts 
$select_about = "SELECT * FROM abouts ";
$select_about_res = mysqli_query($db_connect,$select_about);
$after_about_assoc = mysqli_fetch_assoc($select_about_res);

//for expertise
$skills_query = "SELECT * FROM expertises WHERE status='active'";
...
// protfolio
$protfolio_query = "SELECT * FROM protfolios WHERE status='active'";
$protfolios = mysqli_query($db_connect,$protfolio_query);

//team about
$select = "SELECT * FROM teams WHERE status='active'";
$select_res = mysqli_query($db_connect,$select);
...
<nav class="navbar navbar-expand-lg bg-transprent py-4 fixed-top navigation" id="navbar">
	<div class="container">
	  <a class="navbar-brand" href="index.html">
	  	<img width="200" height="60" src="../sage/logo/logos/<?=$after_assoc['header']?>" />
	  </a>
	  ...
	  <div class="collapse navbar-collapse text-center" id="navbarsExample09">
			<ul class="navbar-nav ml-auto">
			   <li class="nav-item"><a class="nav-link smoth-scroll" href="index.php">Home</a></li>
		...
		<div class="row align-items-center">
			<div class="col-lg-5 col-sm-12 col-12 col-md-5">
				<div class="slider-img position-relative">
					<img src="/sage/about/image/<?=$after_about_assoc['image']?>" alt="" class="img-fluid w-100">
				</div>
			</div>

			<div class="col-lg-6 col-12 col-md-7">
				<div class="ml-5 position-relative mt-5 mt-lg-0">
					<span class="head-trans"><?=$after_about_assoc['watertext']?></span>
					<h1 class="font-weight-normal text-color text-md"><i class="ti-minus mr-2"></i><?=$after_about_assoc['designation']?></h1>
					<h2 class="mt-3 text-lg mb-3 text-capitalize"><?=$after_about_assoc['name']?></h2>
					<p class="animated fadeInUp lead mt-4 mb-5 text-white-50 lh-35"><?=$after_about_assoc['about']?></p>
		...
		<?php foreach ($skills as $skill): ?>
			<div class="col-lg-6 col-md-6">
				<div class="skill-bar mb-4 mb-lg-0">
					<div class="mb-4 text-right"><h4 class="font-weight-normal"><?=$skill['title']?></h4></div>
					<div class="progress">
						<div class="progress-bar" data-percent="<?=$skill['rate']?>">
							<span class="percent-text"><span class="count"><?=$skill['rate']?></span>%</span>
						</div>
					</div>
				</div>
			</div>
			...
			<?php foreach ($protfolios as $protfolio) { ?>
				<div class="item">
					<div class="portfolio-item position-relative">
						<img src="protfolio/image/<?=$protfolio['photo']?>" alt="" class="img-fluid">
					...
					<div class="text mt-3">
						<h4 class="mb-1 text-capitalize"><?=$protfolio['catagory']?></h4>
						<p class="text-uppercase letter-spacing text-sm"><?=$protfolio['title']?></p>
					</div>
				</div>
			<?php } ?>
			</div>
		...
		<div class="row justify-content-center">
			<div class="col-lg-10">
				<div class="testimonial-slider">
					<?php foreach ($select_res as $team) { ?>
						<div class="testimonial-item position-relative">
						<i class="ti-quote-left text-white-50"></i>
						<div class="testimonial-content">
							<p class="text-md mt-3"><?=$team['about']?></p>

							<div class="media mt-5 align-items-center">
								<img src="../sage/team/image/<?=$team['photo']?>" alt="" class="img-fluid  rounded-circle align-self-center mr-4">
								<div class="media-body">
									<h3 class="mb-0"><?=$team['name']?></h3>
									<span class="text-muted"><?=$team['title']?></span>
								</div>
							</div>
						</div>
					</div>
...
<section class="section" id="contact" data-aos="fade-up">
	<div class="container">
		...
		<div class="row justify-content-center">
			<div class="col-lg-8">
				<form class="contact__form form-row contact-form" method="post" action="/sage/message/contact_post.php" id="contactForm">
					...
					<div class="form-group col-lg-12 mb-5">
						...
						<div class="text-center">
							 <input class="btn btn-main text-white mt-5" id="submit" name="submit" type="submit" class="btn btn-hero" value="<?=(isset($_SESSION['done']))?'message send success fully':'send'?>">
						</div>
					</div>
				</form>
			</div>
...
<footer class="footer border-top-1">
	<div class="container">
		<div class="row align-items-center text-center text-lg-left">
			<div class="col-lg-2">
			<img width="200" height="60" src="../sage/logo/logos/<?=$after_assoc['footer']?>" />
			</div>
			<div class="col-lg-10">
