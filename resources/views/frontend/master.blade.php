...
<body>
                        ...
                        <div class="col-lg-4 col-12">
                            <div class="middle-right">
                                <ul>
                                    ...
                                    @auth('customer')
                                    <li><a href="{{ route('customer.profile') }}"><i class="fi flaticon-user-profile"></i></a></li>
                                    <li><a href="{{ route('customer.logout.post') }}"><span>Logout</span></a></li>
                                    @else
                                        ...
                                        <div class="mini-cart">
                                            <button class="cart-toggle-btn"> <i class="fi flaticon-add-to-cart"></i>
                                                <span class="cart-count">
                                                    @auth('customer')
                                                    {{ Auth::guard('customer')->user()->rel_to_cart->count()  }}
                                                    @else
                                                    0
                                                    @endauth
                                                </span></button>
                                            <div class="mini-cart-content">
                                                ...
                                                <div class="mini-cart-items">
                                                    ...
                                                    @endphp
                                                    @auth('customer')
                                                    @foreach (Auth::guard('customer')->user()->rel_to_cart as $cart)
                                                    <div class="mini-cart-item clearfix">
                                                        <div class="mini-cart-item-image">
                                                            <a href="product.html"><img src="{{ asset('uploads/product/preview') }}/{{ $cart->rel_to_product->product_photo }}" alt></a>
                                                        </div>
                                                        <div class="mini-cart-item-des">
                                                            <a href="product.html">{{ $cart->rel_to_product->product_name }}</a>
                                                            <span class="mini-cart-item-price">{{ $cart->rel_to_product->rel_to_inventory->where('color_id', $cart->color_id)->where('size_id', $cart->size_id)->first()->after_discount }} x {{ $cart->quantity }}</span>
                                                            <span class="mini-cart-item-quantity"><a href="{{ route('cart.remove', $cart->id) }}"><i class="ti-close"></i></a></span>
                                                            <span>Color:{{ $cart->rel_to_color->color_name }} Size:{{ $cart->rel_to_size->size_name }} </span>
                                                        </div>
                                                    </div>
                                                    @php
                                                        $sub_total += $cart->rel_to_product->rel_to_inventory->where('color_id', $cart->color_id)->where('size_id', $cart->size_id)->first()->after_discount * $cart->quantity;
                                                    @endphp
                                                    @endforeach
                                                    @endauth
                                                </div>
                                    ...
                                    <div class="mini-shop-item">
                                        <ul id="metis-menu">
                                            ...
                                            <li class="header-catagory-item">
                                                <a class="menu-down-{{ App\Models\Subcategory::where('category_id', $category->id)->exists()?'arrow':'' }}" href="#">{{ $category->category_name }}</a>

                                                @if (App\Models\Subcategory::where('category_id', $category->id)->exists())
                                                <ul class="header-catagory-single">
                                                    @foreach (App\Models\Subcategory::where('category_id', $category->id)->get() as $subcategory)
                                                    <li><a href="{{ route('subcategory.all', $subcategory->subcategory_slug) }}">{{ $subcategory->subcategory_name }}</a></li>
                                                    @endforeach
                                                </ul>
                                                @endif
    ...
    <script src="https://kit.fontawesome.com/30515f1b59.js" crossorigin="anonymous"></script>
